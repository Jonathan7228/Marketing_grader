...
function um_submit_form_errors_hook_login( $args ) {
	...
	if ( isset( $args['username'] ) && $args['username'] == '' ) {
		UM()->form()->add_error( 'username', __( 'Por favor ingrese su usuario o email', 'ultimate-member' ) );
	}

	if ( isset( $args['user_login'] ) && $args['user_login'] == '' ) {
		UM()->form()->add_error( 'user_login', __( 'Por favor ingrese su usuario', 'ultimate-member' ) );
	}

	if ( isset( $args['user_email'] ) && $args['user_email'] == '' ) {
		UM()->form()->add_error( 'user_email', __( 'Por favor ingrese su email', 'ultimate-member' ) );
	}
	...
	if ( $args['user_password'] == '' ) {
		UM()->form()->add_error( 'user_password', __( 'Por favor ingrese su contraseña', 'ultimate-member' ) );
	}
	...
	if ( $user && wp_check_password( $args['user_password'], $user->data->user_pass, $user->ID ) ) {
		...
		UM()->form()->add_error( 'user_password', __( 'Contraseña incorrecta. Intenta de nuevo por favor.', 'ultimate-member' ) );
	}
...
function um_user_login( $args ) {
	...
	if ( ( UM()->options()->get('deny_admin_frontend_login')   && ! isset( $_GET['provider'] ) ) && strrpos( um_user('wp_roles' ), 'administrator' ) !== false ) {
		wp_die( __('Esta acción ha sido impedida por medidas de seguridad.','ultimate-member') );
	}
